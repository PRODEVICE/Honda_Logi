...
Public Class F_Print_Sub_Mitsumori2
    ...
    Private Sub Btn_Output_Click(sender As Object, e As EventArgs) Handles Btn_Output.Click
            ...
            Using conn As New SqlConnection(connectionString)
                ...
                Using tran As SqlTransaction = conn.BeginTransaction()
                    ...
                    Try
                        ...
                        Dim mitsumori_no As String = Cmb_Target.SelectedValue
                        Dim new_mitsumori_no As String = ta_rireki.Q_最新見積No取得("2")

                        If mitsumori_no <> new_mitsumori_no Then
                            ...
                            If MessageBox.Show("過去データが選択されました。１Lot作成時のマスタを呼び出しますか？", "過去マスタ参照", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                                ...
                                fnc.Master_Change_Start(new_mitsumori_no, mitsumori_no, conn, tran)

                            End If

                        End If

                        Using cmd As New SqlCommand("Proc2_4_見積書_機種_見積依頼", conn, tran)
                        ...
                        ' Excel処理にコネクションとトランザクションを渡す
                        ExportToExcel(dt, keisu_flg, 2, conn, tran)
        ...
        Catch ex As Exception
            fnc.ERR_LOG(ex.Message, "F_Print_Sub_Mitsumori2_Btn_Output_Click")
            MessageBox.Show(ex.Message)
        Finally
            '元に戻す
    ...
    '見積書(機種)作成処理
    Private Sub ExportToExcel(dt As DataTable, _keisu_flg As Boolean, _mode As Integer, conn As SqlConnection, tran As SqlTransaction)
        ...
        Try
            ...
            templatePath = IO.Path.Combine(Application.StartupPath, "Excel_Format\見積書(機種)_見積依頼.xlsx")
            file_name = "見積書(機種)_見積依頼_" & nendo & "年度.xlsx"
            ...
            ta_keisu.Connection = conn
            ta_keisu.Transaction = tran

            ta_rate.Connection = conn
            ta_rate.Transaction = tran
            ...
            Using wb As New XLWorkbook(templatePath)
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    ws.Cell(currentRow, 10).Value = If(IsDBNull(row("Col9_2")), "", row("Col9_2").ToString)
                    ws.Cell(currentRow, 11).Value = If(IsDBNull(row("Col10")), "", row("Col10").ToString)
                    ws.Cell(currentRow, 12).Value = If(IsDBNull(row("Col10_2")), "", row("Col10_2").ToString)
                    ws.Cell(currentRow, 13).Value = choku_kousu
                    ws.Cell(currentRow, 14).Value = junbi_kousu
                    ws.Cell(currentRow, 15).Value = kosou_kousu
                    ws.Cell(currentRow, 16).Value = naisou_kousu
                    ws.Cell(currentRow, 17).Value = gaisou_kousu
                    ws.Cell(currentRow, 18).Value = shizai_hi
                    ws.Cell(currentRow, 19).Value = koutin
                    ws.Cell(currentRow, 20).Value = koutin_kanri_hi
                    ws.Cell(currentRow, 21).Value = total_sum
                ...
                ' B列～I列を結合して「合計」文字
                ' -------------------------------
                Dim mergeRange = ws.Range(totalRow, 2, totalRow, 12) ' B=2, L=12
                mergeRange.Merge()
                mergeRange.Value = "合計"
                ...
                ' SUM関数を入れる
                ' -------------------------------
                For col As Integer = 13 To 21 ' M=13, U=21
                ...
                ' 集計行全体に罫線
                ' -------------------------------
                ws.Range(totalRow, 2, totalRow, 21).Style.Border.OutsideBorder = XLBorderStyleValues.Thin
                ws.Range(totalRow, 2, totalRow, 21).Style.Border.InsideBorder = XLBorderStyleValues.Thin
